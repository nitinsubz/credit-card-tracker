...
async function getCreditsToUse(period: Period): Promise<CreditToUse[]> {
  const db = getDb();
  if (!db) {
  ...
  const creditsToUse: CreditToUse[] = [];
  ...
  for (const doc of snapshot.docs) {
    ...
    const card: CreditCard = {
      ...
      benefits: (data?.benefits ?? []).map((b: Benefit) => ({
        ...b,
...
function formatCreditsOutput(credits: CreditToUse[]): string {
  if (credits.length === 0) {
  ...
  'get_credits_to_use',
  {
    ...
    inputSchema: {
      period: z
        ...
        .describe(
          'Time period: "week" = monthly benefits (use before end of month), "month" = monthly, "quarter" = monthly + quarterly, "year" = all benefit types'
        ),
    },
  },
  async ({ period }) => {
